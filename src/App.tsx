...
export default function App() {
            ...
            if (status === "syncing") {
                if (progress) {
                    if (progress.phase === "messages") {
                        setSyncStatus(t("sync.syncing_messages", { current: progress.current, total: progress.total }));
                    } else if (progress.phase === "labels") {
                        setSyncStatus(t("sync.syncing_labels"));
                    } else if (progress.phase === "threads") {
                        setSyncStatus(t("sync.building_threads", { current: progress.current, total: progress.total }));
                    }
                } else {
                    setSyncStatus(t("sync.syncing"));
                }
            } else if (status === "done") {
                setSyncStatus(t("sync.complete"));
                ...
                setSyncStatus(error ? t("sync.failed_with_error", { error: formatSyncError(error) }) : t("sync.failed"));
        ...
        return (
            <div className="flex h-screen items-center justify-center bg-bg-primary">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <span className="text-xs text-text-tertiary animate-pulse">{t("common.loading_inbox")}</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-screen overflow-hidden text-text-primary">
            ...
            {syncStatus && (
                <div
                    className={`fixed bottom-0 left-0 right-0 glass-panel text-white text-xs px-4 py-1.5 text-center z-40 animate-[slideUp_200ms_ease-out,fadeIn_200ms_ease-out] ${syncStatus.startsWith("Sync failed") ? "bg-danger/90" : "bg-accent/90"
                        }`}
                >
                    {syncStatus}
                </div>
